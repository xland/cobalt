...
SbSocketError Socket::Connect(const SbSocketAddress* address) {
  ...
  if (address->type == kSbSocketAddressTypeIpv6) {
    struct sockaddr_in6* saddr_in6 = reinterpret_cast<struct sockaddr_in6*>(&saddr);
    length = sizeof(struct sockaddr_in6);
    saddr_in6->sin6_family = AF_INET6;
    saddr_in6->sin6_port = htons(address->port);
    memcpy(&saddr_in6->sin6_addr, address->address,
  ...
  if (address->type == kSbSocketAddressTypeIpv4) {
    ...
    memcpy(&saddr_in->sin_addr, address->address,
           sizeof(struct in_addr));
  }

  if (::connect(socket_, reinterpret_cast<const struct sockaddr*>(&saddr), length) == 0) {
    return kSbSocketOk;
  }
  return kSbSocketErrorFailed;
#else
...
SbSocketError Socket::Bind(const SbSocketAddress* address) {
  ...
  if (::bind(socket_, reinterpret_cast<const struct sockaddr*>(&saddr), length) == 0) {
    return kSbSocketOk;
  }
  return kSbSocketErrorFailed;
#else
...
int Socket::ReceiveFrom(char* out_data,
  ...
  if (out_source) {
    ...
    if (saddr.sa_family == AF_INET) {
      ...
      memcpy(&out_source->address, &addr_in->sin_addr,
             sizeof(struct in_addr));
    }
#if SB_HAS(IPV6)
  ...
  return -1;
#else
  return SbSocketReceiveFrom(socket_, 
                             out_data, data_size, out_source);
#endif  // SB_API_VERSION >= 16
}
...
int Socket::SendTo(const char* data,
  ...
  ssize_t bytes_written =
        send(socket_, data, data_size, kSendFlags);
  if (bytes_written >= 0) {
...
bool Socket::GetLocalAddress(SbSocketAddress* out_address) {
  ...
  if (out_address) {
    ...
    if (saddr.sa_family == AF_INET) {
      ...
      memcpy(&out_address->address, &addr_in->sin_addr,
             sizeof(struct in_addr));
    }
#if SB_HAS(IPV6)
    if (saddr.sa_family == AF_INET6){
...
bool Socket::SetReuseAddress(bool value) {
  ...
  if (setsockopt(socket_, SOL_SOCKET, SO_REUSEADDR, &on,
                 sizeof(on)) == 0) {
    return true;
  }
  return false;
...
bool Socket::SetTcpKeepAlive(bool value, int64_t period) {
  ...
  if (value){
    ...
    result |= setsockopt(socket_, SOL_TCP, TCP_KEEPIDLE, &period,
                       sizeof(period));
    result |= setsockopt(socket_, SOL_TCP, TCP_KEEPINTVL, &period,
                       sizeof(period));
    if (result != 0) {
      return false;
    }
...
SbSocket SbSocketDbGetSbSocketFromFd(int socket_fd) {
  //return (SbSocket)socketDbGetSbSocket(socket_fd);
  return nullptr;
}

int SbSocketDbGetFdFromSbSocket(SbSocket sb_socket) {
  return -1;
}